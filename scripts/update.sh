...
if [[ -n "$SOURCE_PATH" && -d "$SOURCE_PATH" ]]; then
    (cd "$TARGET_DIR" && composer config repositories.pltx-theme "{\"type\":\"path\",\"url\":\"$SOURCE_PATH\",\"options\":{\"versions\":{\"$PACKAGE_NAME\":\"1.0.0\"},\"symlink\":false}}" && composer update "$PACKAGE_NAME" --no-interaction)
else
